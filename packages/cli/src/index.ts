...
async function runHeadless(opts: CliOptions): Promise<void> {
	...
	console.log(
		`[cli] headless mode — serving at ${running.url} (Ctrl+C to stop)`,
	);
}
...
async function spawnServerChild(opts: {
	host: string;
	port: number;
...
async function runWebview(opts: CliOptions): Promise<void> {
	...
	try {
		...
		console.error(
			"[cli] could not load native webview bindings:",
...
const program = new Command()
	...
	.option(
		"--devtools",
		"enable webview devtools (right-click → Inspect Element)",
	)
