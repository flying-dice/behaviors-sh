...
function decide(req: ReturnType<typeof coreNext>): Verb {
	...
	if (req.type === 'evaluate') {
		...
		if (expr.includes('"morning"')) return { kind: 'eval', result: tod === 'morning' }
		if (expr.includes('"afternoon"')) return { kind: 'eval', result: tod === 'afternoon' }
		if (expr.includes('"evening"')) return { kind: 'eval', result: tod === 'evening' }
		// "is set" gates etc.
...
while (safety-- > 0) {
	...
	if (!('type' in req)) throw new Error(`unexpected response: ${JSON.stringify(req)}`)

	const tag = req.type === 'evaluate' ? `evaluate ${req.name}` : `instruct ${req.name}`
