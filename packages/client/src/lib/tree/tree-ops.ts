...
export function getAt(root: BehaviourNode, path: Path): BehaviourNode {
	...
	for (const i of path) {
		if (isLeaf(node)) {
			throw new Error(
				`Path runs through leaf node at depth ${path.indexOf(i)}.`,
			);
		}
...
export function setCompositeType(
	node: BehaviourNode,
	type: CompositeType,
): BehaviourNode {
...
export function moveStep(
	...
	if (
		...
		from < 0 ||
		to < 0 ||
		from >= node.steps.length ||
		to >= node.steps.length
	)
		return node;
	const steps = node.steps.slice();
