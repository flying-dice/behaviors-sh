...
function place<T>(
    ...
    const cw = m.children.length
        ? m.children.reduce((s, k) => s + k.w, 0) + (m.children.length - 1) * config.hGap
        : 0;
    let cx = x + (m.w - cw) / 2;
    let myCx: number;
    ...
    out.push({ node: m.node, x: myCx - config.nodeW / 2, y, cx: myCx, cy: y + config.nodeH / 2 });
...
export function miniLayout<T>(root: T, config: MiniLayoutConfig<T>): MiniLayoutResult<T> {
    ...
    const width = Math.max(...items.map((it) => it.x + config.nodeW), config.nodeW);
    const height = Math.max(...items.map((it) => it.y + config.nodeH), config.nodeH);
