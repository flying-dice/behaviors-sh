...
export const MARKETPLACE: MarketItem[] = [
	...
	},
	{
		...
		blurb:
			"Classify the inbound ticket, fetch context, hand off or auto-reply.",
		description:
		...
		shape: {
			...
			children: [
				{
					type: "sequence",
					children: [{ type: "instruct" }, { type: "instruct" }],
				},
				{
					type: "sequence",
					children: [
	...
	},
	{
		...
		description:
			"Three-prong audit pass. Produces a Markdown report keyed by severity.",
		colorHint: "purple",
		shape: {
			type: "sequence",
			children: [
...
export function synthShape(id: string): Shape {
	...
	const build = (depth: number): Shape => {
		...
		return {
			type: t,
			children: Array.from({ length: n }, () => build(depth + 1)),
		};
	};
	return {
		type: rand() < 0.5 ? "sequence" : "selector",
		children: Array.from({ length: 2 + Math.floor(rand() * 3) }, () =>
			build(1),
		),
	};
}
