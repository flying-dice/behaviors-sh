...
export function startServer(opts: StartServerOptions = {}): RunningServer {
  ...
  const clientDist = useEmbedded ? '' : resolve(opts.clientDist ?? DEFAULT_CLIENT_DIST)
  ...
  if (useEmbedded) log(`[server]   serving ${Object.keys(opts.embeddedAssets!).length} embedded asset(s)`)
