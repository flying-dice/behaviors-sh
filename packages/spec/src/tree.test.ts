...
describe("BehaviourNodeSchema.toJSONSchema", () => {
	...
	test("declares the draft-2020-12 dialect via zod, not by hand", () => {
		...
		expect(schema.$id).toBe(
			"https://behaviors-sh.dev/schemas/tree.schema.json",
		);
...
describe("StepSchema", () => {
	test("accepts an evaluate step", () => {
		expect(StepSchema.parse({ evaluate: "x > 0" })).toEqual({
			evaluate: "x > 0",
		});
	});
...
describe("ActionNodeSchema", () => {
	...
	test("rejects retries < 1 and non-integer retries", () => {
		...
		expect(ActionNodeSchema.safeParse({ ...base, retries: 0 }).success).toBe(
			false,
		);
...
describe("CompositeNodeSchema", () => {
	test.each([
		...
		"selector",
		"parallel",
	] as const)("parses a %s with one action child", (type) => {
		const node = {
			type,
			name: `${type}-root`,
			children: [
