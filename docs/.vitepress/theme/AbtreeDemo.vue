...
const yaml: YamlLine[] = [
	...
	{
		text: "            Store pass/fail at $VAR.tests_passed.",
		cls: "yi",
		node: "A",
	},
	{
		text: "            Store coverage percentage at $VAR.coverage.",
		cls: "yi",
		node: "A",
	},
	{ text: "        - evaluate: |", cls: "ye", node: "A", step: "evaluate" },
	{ text: "            $VAR.tests_passed is true.", cls: "ye", node: "A" },
	{
		text: "            $VAR.coverage is greater than $CONST.threshold.",
		cls: "ye",
		node: "A",
	},
	...
	{
		text: "            Build and push image to $CONST.registry.",
		cls: "yi",
		node: "B",
	},
	{
		text: "            Store the pushed tag at $VAR.image_tag.",
		cls: "yi",
		node: "B",
	},
	{ text: "state:", cls: "yk" },
...
const frames: Frame[] = [
	// Create the execution
	{
		node: null,
		lines: [
			{ text: "start_execution(tree_uri, trace_output)", kind: "cmd" },
			{ text: '{"phase":"protocol"}', kind: "resp" },
			{ text: "", kind: "blank" },
		],
	},
	// A — Run_Tests: instruct returned
	{
		...
		lines: [
			{ text: "next_step(trace_output)", kind: "cmd" },
			{
	...
	// A — do the work, write results, submit (more steps remain → step_complete)
	{
		...
		lines: [
			{
				text: "▸ run test suite → store at $VAR.tests_passed and $VAR.coverage",
				kind: "think",
			},
			{ text: "$ bun test --coverage", kind: "cmd" },
			{ text: "All tests passed. Coverage: 87%", kind: "resp" },
			{ text: "", kind: "blank" },
			{ text: "▸ writing results to $VAR", kind: "think" },
			{
				text: 'var_write(trace_output, "tests_passed", "true")',
				kind: "cmd",
			},
			{ text: 'var_write(trace_output, "coverage", "87")', kind: "cmd" },
			{ text: 'submit(trace_output, "success")', kind: "cmd" },
	...
	// A — gate evaluate returned
	{
		...
		lines: [
			{ text: "next_step(trace_output)", kind: "cmd" },
			{
				text: '{"type":"evaluate","name":"Run_Tests","expression":"$VAR.tests_passed is true.\\n$VAR.coverage is greater than $CONST.threshold."}',
				kind: "resp",
			},
			{ text: "", kind: "blank" },
		],
	...
	// A — read all three refs, eval combined expression, completes A
	{
		...
		lines: [
			{
				text: "▸ refs: $VAR.tests_passed, $VAR.coverage, $CONST.threshold",
				kind: "think",
			},
			{ text: 'var_read(trace_output, "tests_passed")', kind: "cmd" },
			{ text: '{"path":"tests_passed","value":true}', kind: "resp" },
			{ text: 'var_read(trace_output, "coverage")', kind: "cmd" },
			{ text: '{"path":"coverage","value":87}', kind: "resp" },
			{ text: 'const_read(trace_output, "threshold")', kind: "cmd" },
			{ text: '{"path":"threshold","value":80}', kind: "resp" },
			{ text: "", kind: "blank" },
			{ text: "▸ true and 87 > 80 → pass", kind: "think" },
			{ text: "eval(trace_output, true)", kind: "cmd" },
	...
	// B — Build_And_Push: instruct returned
	{
		...
		lines: [
			{ text: "next_step(trace_output)", kind: "cmd" },
			{
				text: '{"type":"instruct","name":"Build_And_Push","instruction":"Build and push image to $CONST.registry.\\nStore the pushed tag at $VAR.image_tag."}',
				kind: "resp",
			},
			{ text: "", kind: "blank" },
		],
	...
	// B — read registry, build, write image_tag, submit, then next → done
	{
		...
		completesAs: "success",
		lines: [
			{ text: "▸ resolve $CONST.registry before tagging", kind: "think" },
			{ text: 'const_read(trace_output, "registry")', kind: "cmd" },
			...
			{ text: "", kind: "blank" },
			{
				text: 'var_write(trace_output, "image_tag", "ghcr.io/my-app:abc1234")',
				kind: "cmd",
			},
			{ text: 'submit(trace_output, "success")', kind: "cmd" },
